...
class AUCMeter(Meter):
    ...
    def value(self, max_fpr=1.0):
        ...
        for i in range(1, scores.size + 1):
            if self.targets[sortind[i - 1]] == 1: #a true positive example 
                tpr[i] = tpr[i - 1] + 1 
                fpr[i] = fpr[i - 1]
            else:
                tpr[i] = tpr[i - 1]
        ...
        tpr /= (self.targets.sum() * 1.0) #divide by true positive examples 
        fpr /= ((self.targets - 1.0).sum() * -1.0) #divide by true negative examples
